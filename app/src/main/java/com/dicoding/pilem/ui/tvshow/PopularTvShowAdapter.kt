...
import com.dicoding.pilem.databinding.TvshowItemRowBinding
import com.dicoding.pilem.util.imageHelper.imageLoader
import kotlinx.coroutines.Dispatchers
import kotlinx.coroutines.GlobalScope
import kotlinx.coroutines.launch

class PopularTvShowAdapter: ListAdapter<FilmData, PopularTvShowViewHolder>(PopularTvShowDiffUtil()){
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): PopularTvShowViewHolder {
        val inflater = LayoutInflater.from(parent.context)
        val popularMovieView = TvshowItemRowBinding.inflate(inflater, parent, false)
        return PopularTvShowViewHolder(popularMovieView)
    }
...
class PopularTvShowViewHolder(private val binding : TvshowItemRowBinding):RecyclerView.ViewHolder(binding.root){
...
class PopularTvShowDiffUtil:DiffUtil.ItemCallback<FilmData>() {
